...
#### 中间件消息追踪规则（强制）
- **消息通道清单**：检索所有中间件（xnet/webnet/TDMQ/RabbitMQ/IBMMQ/Kafka/RocketMQ等）的 Topic/Group/Queue 名称，生成消息通道清单
- **跨应用数据流**：根据消息通道追踪数据在应用间的流动路径，明确生产者→消费者对应关系
- **幽灵通道检测**：标记只有生产者没有消费者的 Topic/Queue
- **孤儿通道检测**：标记只有消费者没有生产者的 Topic/Queue
- **消息体格式记录**：记录消息体的关键字段和结构
