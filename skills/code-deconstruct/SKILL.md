...
| 中间件类型 | 识别特征 | 关注点 |
| ---------- | ---------- | -------- |
| xnet | xnet/Topic/Group | 应用间数据流动追踪 |
| webnet | webnet/Title/Group | 跨应用消息路由 |
| TDMQ | TDMQ/Topic/Subscription | 腾讯云消息队列 |
| RabbitMQ | RabbitMQ/Exchange/Queue/Binding | 消息路由拓扑 |
| IBMMQ | IBMMQ/QueueManager/Queue | IBM消息队列 |
| RocketMQ | RocketMQ/Topic/ConsumerGroup | 阿里云消息队列 |
| Kafka | Kafka/Topic/ConsumerGroup | 高吞吐消息队列 |
| 其他MQ | MQ/Message/Producer/Consumer | 自定义消息中间件 |

### 2. 消息追踪清单

扫描代码中所有中间件相关的配置，提取消息通道信息：

#### 扫描目标

- 配置文件：`*.yml`、`*.yaml`、`*.properties`、`*.xml`、`*.json`
- 注解配置：`@XnetListener`、`@RabbitListener`、`@JmsListener`、`@KafkaListener` 等
- 编程式API：`channel.subscribe()`、`consumer.receive()`、`producer.send()` 等
- 硬编码字符串：`new Topic("xxx")`、`new Queue("xxx")`、`topic = "xxx"` 等

#### 生成消息追踪清单

生成文件 `docs/deconstruct/middleware_tracing.md`：
...
| 依赖关系 | 依赖方 | 被依赖方 | 通道名称 | 影响范围 |
|----------|--------|----------|----------|----------|
| 强依赖 | 应用A | 应用B | T: order_create | 应用B不可用则应用A阻塞 |
| 弱依赖 | 应用A | 应用C | T: notify | 应用C不可用不影响主流程 |

### 4. 数据流追踪要点

1. **Topic/Group/Queue 名称必须记录**：所有消息通道的名称必须记录在案
2. **跨应用数据流向必须明确**：标识数据从哪个应用流向哪个应用
3. **消息体格式必须记录**：即使是 JSON 也要记录关键字段
4. **生产者消费者必须对应**：确保每个 Topic 都有对应的生产者和消费者
5. **缺失的消费者标记**：发现只有生产者没有消费者的情况，标记为"幽灵通道"
6. **缺失的生产者标记**：发现只有消费者没有生产者的情况，标记为"孤儿通道"
...
| 通信类型 | 识别特征 | 关注点 |
| ---------- | ---------- | -------- |
| HTTP API | HttpClient/RestTemplate/Feign | 请求频率、响应大小、超时设置 |
| RPC调用 | gRPC/Dubbo/Thrift | 序列化大小、连接池配置 |
| 消息队列 | Kafka/RabbitMQ/RocketMQ | 消息大小、吞吐量、网络延迟 |
| 数据库连接 | JDBC/ORM连接池 | 连接数、查询频率、结果集大小 |
| 文件传输 | FTP/SFTP/HTTP下载 | 文件大小、传输速率、断点续传 |
| WebSocket | 长连接通信 | 连接数、消息频率、心跳间隔 |
| 缓存同步 | Redis/Memcached | 同步频率、数据量、网络拓扑 |
| 服务发现 | Consul/Etcd/Nacos | 心跳频率、服务元数据大小 |
...
| 估算维度 | 计算方法 | 单位 |
| ---------- | ---------- | ------ |
| 请求大小 | 请求头 + 请求体序列化大小 | KB |
| 响应大小 | 响应头 + 响应体序列化大小 | KB |
| 并发请求 | QPS × 平均请求大小 × 8 | Mbps |
| 响应带宽 | QPS × 平均响应大小 × 8 | Mbps |
| 上行流量 | 日请求总量 × 请求大小 | GB/天 |
| 下行流量 | 日请求总量 × 响应大小 | GB/天 |
| 总带宽 | (上行流量 + 下行流量) × 8 / 86400 | Mbps |
...
| 接口名称 | QPS | 平均请求大小 | 平均响应大小 | 上行带宽 | 下行带宽 | 总带宽 |
|----------|-----|--------------|--------------|----------|----------|--------|
| 用户注册 | 100 | 2KB | 1KB | 1.6Mbps | 0.8Mbps | 2.4Mbps |
| 订单查询 | 500 | 1KB | 5KB | 4Mbps | 20Mbps | 24Mbps |
| 文件上传 | 50 | 10MB | 0.5KB | 4000Mbps | 0.2Mbps | 4000.2Mbps |

### RPC接口性能分析

| 服务方法 | 调用频率 | 序列化大小 | 网络延迟 | 超时设置 | 成功率 |
|----------|----------|------------|----------|----------|--------|
| getUserInfo | 1000次/秒 | 1KB | 10ms | 1000ms | 99.9% |
| createOrder | 200次/秒 | 2KB | 20ms | 2000ms | 99.5% |
| batchProcess | 50次/秒 | 10KB | 100ms | 5000ms | 99.0% |
```

### 4. 网络通信文档结构

文件：`docs/deconstruct/network_usage.md`
